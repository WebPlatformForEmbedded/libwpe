...
{
    int min = 0;
    int max = sizeof(keysym_to_unicode_table) / sizeof(keysym_to_unicode_table[0]) - 1;
    int mid;

    /* First check for Latin-1 characters (1:1 mapping) */
    if ((key_code >= 0x0020 && key_code <= 0x007e) || (key_code >= 0x00a0 && key_code <= 0x00ff))
        return key_code;

    /* Also check for directly encoded 24-bit UCS characters */
    if ((key_code & 0xff000000) == 0x01000000)
        return key_code & 0x00ffffff;

    /* binary search in table */
    while (max >= min) {
        mid = (min + max) / 2;
        if (keysym_to_unicode_table[mid].keysym < key_code)
            min = mid + 1;
        else if (keysym_to_unicode_table[mid].keysym > key_code)
            max = mid - 1;
        else /* found it */
            return keysym_to_unicode_table[mid].ucs;
    }
